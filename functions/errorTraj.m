function error = errorTraj(gt_traj,traj)
    ...
    for ii = gt_arrayTime(1,1) : 0.01 : gt_arrayTime(end,1)
     ...
     if gt_newData
        ...
        error(gt_idx,:) = [gt_traj(gt_idx,1) norm(gtXYZ(1) - trajXYZ(1)) norm(gtXYZ(2) - trajXYZ(2)) norm(gtXYZ(3) - trajXYZ(3)) norm(gtXYZ-trajXYZ)];
     end
